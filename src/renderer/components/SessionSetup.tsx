...
export function SessionSetup() {
  ...
  const useCurrentResumeAndStart = async () => {
    if (resumeUploading) return
    setResumeUploading(true)
    setResumeError('')
    try {
      await startSession({
        profile: snapshotLocalProfile(profile ?? undefined),
        usedDefaults: true,
      })
    } catch (error) {
      setResumeError(error instanceof Error ? error.message : 'Could not use your saved resume')
    } finally {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center px-6 py-5">
      <div className="w-full max-w-[360px] text-center">
        ...
        {hasCurrentResume ? (
          <Button className="mt-5 w-full" onClick={() => void useCurrentResumeAndStart()} disabled={resumeUploading} loading={resumeUploading}>
            Use current resume and start session
          </Button>
        ) : (
        ...
        {hasCurrentResume ? (
          <Button variant="outline" className="mt-3 w-full" onClick={() => void uploadAndStart()} disabled={resumeUploading}>
            Upload a different resume
          </Button>
        ) : null}
        <Button variant="outline" className={hasCurrentResume ? 'mt-3 w-full' : 'mt-3 w-full'} onClick={cancelSessionSetup} disabled={resumeUploading}>
          Cancel
        </Button>
