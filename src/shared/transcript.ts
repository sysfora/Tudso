...
export function isUselessRealtimeReply(text: string): boolean {
  const value = normalize(text)
  if (!value) return true
  if (/^no[_\s-]?reply$/i.test(value)) return true
  if (value === '(empty)' || value === '(no response)') return true
  if (isNoiseTranscript(value)) return true
  const compact = value.toLowerCase()
  return (
    compact.includes('audio is noisy') ||
    compact.includes('cannot hear') ||
    compact.includes("can't hear") ||
    compact.includes('no speech') ||
    compact.includes('empty string')
  )
}
